...
MIDDLEWARE = [
    ...
    # 'django.middleware.csrf.CsrfViewMiddleware',  # 为了使用postman进行测试需要关掉csrf防护
...
DATABASES = {
    'default': {
        ...
        'PORT': 3306,  # 数据库端口
        'USER': 'seauning',  # 数据库用户名
        'PASSWORD': '123456',  # 数据库用户密码
        'NAME': 'blog_server'  # 数据库名字
    }
}
...
'''
# 此处需要先配置好 redis 缓存，否则 django 无法找到缓存，会一直占用cpu致使主机卡顿(需要注意linux和windows的配置方式不同)
CACHES = {
    "default": {  # 默认
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://127.0.0.1:6379/0",  # 6379 是 redis 的默认端口
        "OPTIONS": {
...
LOGGING = {
    ...
    'handlers': {  # 日志处理方法
        ...
        'file': {  # 向文件中输出日志
            ...
            'filename': os.path.join(BASE_DIR, 'logs/blog_server.log'),  # 日志文件的位置
